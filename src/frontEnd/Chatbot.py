...
def _build_netlist_prompt(netlist_path: str, raw_lines):
    ...
    return (
        ...
        "Use ONLY the internal facts and active netlist below. Do not use the file name, "
        "project name, or general circuit templates to add components that are not present.\n\n"
        "Rules:\n"
        "- Treat the deterministic internal facts as authoritative over your own interpretation, "
        "but do not expose internal FACT labels to the user.\n"
        "- Do not copy internal labels as headings or bullets. Do not output headings such as "
        "ACTIVE NETLIST, CONTROL BLOCK LINES, CONTROL_BLOCK_LINES, OUTPUT COMMANDS, "
        "OUTPUT_COMMANDS, INCLUDE FILES, INCLUDE_FILES, TRANSACTION DIRECTIVES, TRAN_FIELDS, "
        "VOLTAGE SOURCES, VOLTAGE_SOURCES, LOAD CANDIDATES, or LOAD_CANDIDATES.\n"
        "- Use internal facts only as evidence to write the required five user-facing sections.\n"
        ...
        "- Do not label the circuit as switching, AC-coupled, temperature-controlled, "
        "feedback-controlled, or successful unless those exact features are directly "
        "supported by deterministic facts in COMPONENT_LINES, CONTROL_COMMANDS, "
        "OUTPUT_COMMANDS, or simulation result data.\n"
        ...
        "- Do not mention temperature, thermal control, or temperature compensation for "
        "LM7805 unless an active netlist line or included simulation result explicitly "
        "shows a temperature/thermal model or parameter.\n"
        ...
        "- Use exactly the five section headings below. Do not add a separate conclusion, "
        "summary, or final paragraph outside those headings.\n"
        "- Section 1 should summarize components, nodes, include files, model names, and "
        "subcircuit instances in readable sentences or concise bullets.\n"
        "- Section 2 should explain the .tran directive and .control/run/print/plot/output "
        "commands in readable language.\n"
        "- Section 3 should explain likely circuit intent cautiously in one or two sentences.\n"
        "- Section 4 should list only deterministic possible issues, or say none are obvious "
        "from the provided facts.\n"
        "- Section 5 should explicitly mention unknowns from the netlist alone.\n"
        "- In section 3, use at most two cautious sentences and avoid unsupported labels.\n"
        "- In section 5, explicitly list whether output voltage, regulation stability, "
        "LM7805 internal behavior, temperature behavior, and simulation success are "
        "unknown from the netlist alone unless provided facts prove otherwise.\n\n"
